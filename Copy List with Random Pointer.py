...
class Solution:
    def copyRandomList(self, head: 'Node') -> 'Node':
        ...
        #print([i.next for i in has.values()])
        original = head
        #clone = None
        while original:
            clone = has[original]
            clone.next = None if original.next is None else has[original.next]
            clone.random = None if original.random is None else has[original.random]
            original = original.next
        return has[head]
...
class Solution:
    def copyRandomList(self, head: 'Node') -> 'Node':
        ...
        while node:
            ...
            if node.random.random:
                node.random.random = node.random.random.random
